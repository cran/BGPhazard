...
function(M, fun = "both", confint = TRUE, h.NA = TRUE, KM =TRUE, 
  ...
  if (fun == "h" || fun == "both") {
    ...
    for(k in 1:(K+1)) {
      points(x = k - 1, y = SUM.h[k, 2], pch = 20)
      if (confint == TRUE) {
        points(x = k - 1, y = SUM.h[k, 3], pch="-", col = "darkred")
        points(x = k - 1, y = SUM.h[k, 5], pch="-", col = "darkred")
        segments(x0 = k - 1, y0 = SUM.h[k, 3], x1 = k - 1, y1 = SUM.h[k, 5], 
                 col = "darkred", lty = 2)
      }
    }
    if (legend == TRUE) {
